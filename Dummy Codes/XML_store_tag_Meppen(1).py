...
def MyXML(filePath):
                               ...
                               # print(element[i][j][k][l].tag)
                                ...
                                if(ln4 > 0):
                                    for m in range(ln4):
                                        ...
                                        tag5.append(element[i][j][k][l][m].tag)
                                        ln5 = len(element[i][j][k][l][m])
                                        #print(element[i][j][k][l][m])
    ...
    list4=pd.Series(tag4)
    df = pd.concat([list1,list2,list3,list4], ignore_index=True, axis=1)
    df.columns=["Tag","SubTag","ChildTag","GrandChildTag"]
