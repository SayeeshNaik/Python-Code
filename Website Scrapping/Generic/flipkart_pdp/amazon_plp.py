...
class FlipkartpdpSpider(sp.Spider):
    ...
    main_url = 'https://www.amazon.in/dp/'
    df_product_id = pd.read_excel("AmazonFiles/Product ID.xlsx")
    urls,output_data = [],[]
    for id in df_product_id['product_id']: urls.append(main_url + id)
    df_xpaths = pd.read_excel("AmazonFiles/xpaths.xlsx")
    ...
    def parse(self, response):
        pass
        name = response.xpath(("//span[@id='productTitle']" + '/text()')).getall()
        print("nnnnnnnn = ",name)
        print("pppp = ",self.df_xpaths)
        temp_dict = {}
        try:
