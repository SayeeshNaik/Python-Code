...
class AmazonPlpSpider(sp.Spider):
    ...
    def start_requests(self):
        ...
        #         for i in self.listings[0:50]:                        
        #             myurl = '''https://www.amazon.in/dp/{0}?th=1&psc=1'''.format(str(i))
        #             yield sp.Request(url=myurl, callback=self.parse)
                    
        # except: 
            
             for i in self.listings[0:10]:                        
                myurl = '''https://www.amazon.in/dp/{0}?th=1&psc=1'''.format(
                    str(i))
                yield sp.Request(url=myurl, callback=self.parse)
                # yield SplashRequest(url=myurl, callback=self.parse)
    ...
    def parse(self, response):
        ...
        for k in self.xpaths:
            ...
            val = response.xpath(self.xpaths[k]).extract_first()
        ...
        try:
                if(temp_dict['TITLE']==None): 
                    ...
                    with open("AmazonPage.html","w") as file:
...
exp_df = exp_df['Exp_ID']
global_exp_id_lis = []
for id in exp_df: global_exp_id_lis.append(id)

print("HELLLL))))))))))))))))))))))))))",global_exp_id_lis)

# for exp_id in global_exp_id_lis:
#     print(exp_id)
# process.crawl(AmazonPlpSpider)
# process.start()
